package com.planet_ink.coffee_mud.Libraries;
 com.planet_ink.coffee_mud.core.exceptions.BadEmailAddressException;
 ...
 com.planet_ink.coffee_mud.Common.interfaces.Session.InputCallback;
 com.planet_ink.coffee_mud.Common.interfaces.Session.SessionStatus;
 com.planet_ink.coffee_mud.Exits.interfaces.;
 com.planet_ink.coffee_mud.Items.interfaces.;
 com.planet_ink.coffee_mud.Locales.interfaces.;
 com.planet_ink.coffee_mud.MOBS.interfaces.;
 com.planet_ink.coffee_mud.Races.interfaces.;
 com.planet_ink.miniweb.util.MWThread;
...
public class CharCreation extends StdLibrary implements CharCreationLibrary
{
	...
	protected final String RECONFIRMSTR="nr^WTry entering ^HY^W or ^HN^W: ";
	...
	public void reloadTerminal(MOB mob)
	{
		...
		&&(!CMSecurity.isDisabled(CMSecurity.DisFlag.MXP)))
		{
			...
			{
				StringBuffer mxpText=Resources.getFileResource("textmxp.txt",true);
				if(mxpText!=null)
					S.rawOut("033[6z"+mxpText.toString()+"nr");
			}
			else
	...
	public boolean isExpired(PlayerAccount acct, Session session, long expiration)
	{
		...
		{
			session.println("nr"+CMProps.getVar(CMProps.Str.EXPCONTACTLINE)+"nrnr");
	...
	protected void finishCreateAccount(final LoginSession loginObj, final PlayerAccount acct, final String login, final String pw, final String emailAddy, final Session session)
	{
		...
		if(CMProps.getBoolVar(CMProps.Bool.ACCOUNTEXPIRATION))
			acct.setAccountExpiration(System.currentTimeMillis()+(1000l60l60l24l(CMProps.getIntVar(CMProps.Int.TRIALDAYS))));
		...
		if(((pw==null)||(pw.length()==0))&&(!CMProps.getVar(CMProps.Str.EMAILREQ).startsWith("DISABLE")))
		{
			...
			CMLib.smtp().emailOrJournal(CMProps.getVar(CMProps.Str.SMTPSERVERNAME), acct.accountName(), "noreply@"+CMProps.getVar(CMProps.Str.MUDDOMAIN).toLowerCase(), acct.accountName(),
				...
				"Your password for "+acct.accountName()+" is: "+password+"nrYou can login by pointing your mud client at "+CMProps.getVar(CMProps.Str.MUDDOMAIN)+" port(s):"+CMProps.getVar(CMProps.Str.MUDPORTS)+".nrAfter creating a character, you may use the PASSWORD command to change it once you are online.");
		...
		else
		{
			...
			session.println(null,null,null,"nrnr"+doneText.toString());
		}
	...
	private boolean loginsDisabled(MOB mob)
	{
		...
		{
			StringBuffer rejectText=Resources.getFileResource("textnologins.txt",true);
	...
	public LoginResult prelimChecks(Session session, String login, PlayerLibrary.ThinnerPlayer player)
	{
		...
		{
			session.println("nrYou are unwelcome.  No one likes you here. Go away.nrnr");
			session.stopSession(false,false,false);
	...
	{
		StringBuffer rpt = new StringBuffer("rnMSSP-REPLY-START");
		rpt.append("rn"); rpt.append("PLAYERS");
		rpt.append("t"); rpt.append(Integer.toString(CMLib.sessions().getCountLocalOnline()));
		rpt.append("rn"); rpt.append("STATUS");
		rpt.append("t");
		...
		if(host != null)
		{
			rpt.append("rn"); rpt.append("UPTIME");
			rpt.append("t"); rpt.append(Long.toString(host.getUptimeSecs()));
			rpt.append("rn"); rpt.append("HOSTNAME");
			rpt.append("t"); rpt.append(host.getHost());
			rpt.append("rn"); rpt.append("PORT");
			rpt.append("t"); rpt.append(Integer.toString(host.getPort()));
			if(Thread.currentThread() instanceof MWThread)
			{
				...
				rpt.append("rn"); rpt.append("WEBSITE");
				rpt.append("t"); rpt.append(("http:"+host.getHost()+":"+webServerPort));
			}
			rpt.append("rn"); rpt.append("LANGUAGE");
			rpt.append("t"); rpt.append(host.getLanguage());
		}
		if(CMLib.intermud().i3online())
		{
			rpt.append("rn"); rpt.append("INTERMUD");
			rpt.append("t"); rpt.append("I3");
		}
		if(CMLib.intermud().imc2online())
		{
			rpt.append("rn"); rpt.append("INTERMUD");
			rpt.append("t"); rpt.append("IMC2");
		}
		rpt.append("rn"); rpt.append("FAMILY");
		rpt.append("t"); rpt.append("CoffeeMUD");
		rpt.append("rn"); rpt.append("EMAIL");
		rpt.append("t"); rpt.append(CMProps.getVar(CMProps.Str.ADMINEMAIL));
		rpt.append("rn"); rpt.append("CODEBASE");
		rpt.append("t"); rpt.append(("CoffeeMud v"+CMProps.getVar(CMProps.Str.MUDVER)));
		rpt.append("rn"); rpt.append("AREAS");
		rpt.append("t"); rpt.append(Integer.toString(CMLib.map().numAreas()));
		rpt.append("rn"); rpt.append("HELPFILES");
		rpt.append("t"); rpt.append(Integer.toString(CMLib.help().getHelpFile().size()));
		rpt.append("rn"); rpt.append("MOBILES");
		rpt.append("t"); rpt.append(Long.toString(CMClass.numPrototypes(CMClass.CMObjectType.MOB)));
		rpt.append("rn"); rpt.append("OBJECTS");
		rpt.append("t"); rpt.append(Long.toString(CMClass.numPrototypes(CMClass.OBJECTS_ITEMTYPES)));
		rpt.append("rn"); rpt.append("ROOMS");
		rpt.append("t"); rpt.append(Long.toString(CMLib.map().numRooms()));
		rpt.append("rn"); rpt.append("CLASSES");
		int numClasses = 0;
		if(!CMSecurity.isDisabled(CMSecurity.DisFlag.CLASSES))
			numClasses=CMLib.login().classQualifies(null, CMProps.getIntVar(CMProps.Int.MUDTHEME)&0x07).size();
		rpt.append("t"); rpt.append(Long.toString(numClasses));
		rpt.append("rn"); rpt.append("RACES");
		int numRaces = 0;
		if(!CMSecurity.isDisabled(CMSecurity.DisFlag.RACES))
			numRaces=CMLib.login().raceQualifies(null, CMProps.getIntVar(CMProps.Int.MUDTHEME)&0x07).size();
		rpt.append("t"); rpt.append(Long.toString(numRaces));
		rpt.append("rn"); rpt.append("SKILLS");
		rpt.append("t"); rpt.append(Long.toString(CMLib.ableMapper().numMappedAbilities()));
		rpt.append("rn"); rpt.append("ANSI");
		rpt.append("t"); rpt.append((this!=null?"1":"0"));
		rpt.append("rn"); rpt.append("MCCP");
		rpt.append("t"); rpt.append((!CMSecurity.isDisabled(CMSecurity.DisFlag.MCCP)?"1":"0"));
		rpt.append("rn"); rpt.append("MSP");
		rpt.append("t"); rpt.append((!CMSecurity.isDisabled(CMSecurity.DisFlag.MSP)?"1":"0"));
		rpt.append("rn"); rpt.append("MXP");
		rpt.append("t"); rpt.append((!CMSecurity.isDisabled(CMSecurity.DisFlag.MXP)?"1":"0"));
		rpt.append("rnMSSP-REPLY-ENDrn");
		return rpt.toString();
	}
	...
	public LoginResult loginSystem(Session session, LoginSession loginObj) throws IOException
	{
		...
		{
			session.println("nrnrI'm sorry, but something bad happened. You'll need to re-log.nrnr");
			Log.errOut(e);
		}
		loginObj.reset=true;
		loginObj.state=LoginState.LOGIN_START;
	...
	protected LoginResult loginStart(final LoginSession loginObj, final Session session)
	{
		...
		if(CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM)>1)
			session.promptPrint("nraccount name: ");
		else
			session.promptPrint("nrname: ");
	...
	protected LoginResult loginName(final LoginSession loginObj, final Session session)
	{
		...
		if(CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM)>1)
		{
			...
			else
			{
				...
				{
					session.println("nrAccount '"+CMStrings.capitalizeAndLower(loginObj.login)+"' does not exist.");
	...
	protected LoginResult loginAcctcharPword(final LoginSession loginObj, final Session session)
	{
		...
		if(loginObj.player.matchesPassword(loginObj.password))
		{
			...
			{
				session.println("nrThis mud is now using an account system.  "
						+"Please create a new account and use the IMPORT command to add your character(s) to your account.");
				session.promptPrint("Would you like to create your new master account and call it '"+loginObj.player.name+"' (yN)? ");
				loginObj.state=LoginState.LOGIN_ACCTCONV_CONFIRM;
				return LoginResult.INPUT_REQUIRED;
			}
			else
			{
				session.println("nrThis mud uses an account system.  Your account name is `^H"+loginObj.player.accountName+"^N`.nr"
	...
	protected LoginResult acctcreateStart(final LoginSession loginObj, final Session session)
	{
		...
		session.promptPrint("nrDo you want ANSI colors (Yn)?");
		return LoginResult.INPUT_REQUIRED;
	}
	...
	protected LoginResult acctcreateANSIConfirm(final LoginSession loginObj, final Session session)
	{
		...
		session.println(null,null,null,"nrnr"+introText.toString());
		final boolean emailPassword=((CMProps.getVar(CMProps.Str.EMAILREQ).toUpperCase().startsWith("PASS"))
				 &&(CMProps.getVar(CMProps.Str.MAILBOX).length()>0));
		if(!emailPassword)
		{
			session.promptPrint("nrEnter an account passwordnr: ");
	...
	{
		session.promptPrint("nrEnter your e-mail address: ");
	...
	protected LoginResult acctcreateEmailEntered(final LoginSession loginObj, final Session session)
	{
		...
		{
			session.println("nrA valid email address is required.nr");
			loginObj.state=LoginState.ACCTCREATE_EMAILPROMPT;
			return null;
		}
		...
		if(emailReq||emailPassword)
		{
			session.promptPrint("Confirm that '"+newEmail+"' is correct by re-entering.nrRe-enter: ");
	...
	protected LoginResult acctcreateEmailConfirmed(final LoginSession loginObj, final Session session)
	{
		...
		session.println("nrThat email address combination was invalid.nr");
		loginObj.state=LoginState.ACCTCREATE_EMAILPROMPT;
		return null;
	}
	...
	protected LoginResult acctcreatePassworded(final LoginSession loginObj, final Session session)
	{
		...
		{
			session.println("nrAborting account creation.");
			loginObj.state=LoginState.LOGIN_START;
		}
		else
		{
	...
	protected LoginResult loginPassStart(final LoginSession loginObj, final Session session)
	{
		if(loginObj.player==null)
		{
			if(newCharactersAllowed(loginObj.login,session,loginObj.acct,CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM)<=0))
			{
				if(CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM)>1)
				{
					session.promptPrint("nr'"+CMStrings.capitalizeAndLower(loginObj.login)+"' does not exist.nrIs this a new account you would like to create (yN)?");
					loginObj.state=LoginState.LOGIN_NEWACCOUNT_CONFIRM;
					return LoginResult.INPUT_REQUIRED;
				}
				else
				{
					session.promptPrint("nr'"+CMStrings.capitalizeAndLower(loginObj.login)+"' does not exist.nrIs this a new character you would like to create (yN)?");
	...
	protected LoginResult loginPassReceived(final LoginSession loginObj, final Session session) throws IOException
	{
		...
		else
		{
			...
			session.println("nrInvalid password.nr");
			...
			&&(CMProps.getVar(CMProps.Str.MUDDOMAIN).length()>0))
			{
				if(CMProps.getBoolVar(CMProps.Bool.HASHPASSWORDS))
					session.promptPrint("Would you like you have a new password generated and e-mailed to you (yN)? ");
				else
					session.promptPrint("Would you like your password e-mailed to you (yN)? ");
	...
	protected LoginResult acctmenuShowMenu(final LoginSession loginObj, final Session session)
	{
		...
		if(!acct.isSet(PlayerAccount.FLAG_ACCOUNTMENUSOFF))
		{
			buf.append(" ^XAccount Menu^.^Nnr");
			buf.append(" ^XL^.^w)^Hist charactersnr");
			buf.append(" ^XN^.^w)^Hew characternr");
			if(acct.isSet(PlayerAccount.FLAG_CANEXPORT))
			{
				buf.append(" ^XI^.^w)^Hmport characternr");
				buf.append(" ^XE^.^w)^Hxport characternr");
			}
			buf.append(" ^XD^.^w)^HeleteRetire characternr");
			buf.append(" ^XH^.^w)^Helpnr");
			buf.append(" ^XM^.^w)^Henu OFFnr");
			buf.append(" ^XP^.^w)^Hassword changenr");
			if(!CMProps.getVar(CMProps.Str.EMAILREQ).toUpperCase().startsWith("DISABLE"))
				buf.append(" ^XE^.^w)^Hmail changenr");
			buf.append(" ^XQ^.^w)^Huit (logout)nr");
			buf.append("nr^H ^w(^HEnter your character name to login^w)^H");
			session.println(buf.toString());
			buf.setLength(0);
		}
		loginObj.state=LoginState.ACCTMENU_PROMPT;
	...
	protected LoginResult acctmenuPrompt(final LoginSession loginObj, final Session session)
	{
		...
		session.promptPrint("nr^wCommand or Name ^H(?)^w: ^N");
		loginObj.state=LoginState.ACCTMENU_COMMAND;
		loginObj.savedInput="";
		return LoginResult.INPUT_REQUIRED;
	}
	...
	protected LoginResult acctmenuCommand(final LoginSession loginObj, final Session session) throws IOException
	{
		...
		if(cmd.equalsIgnoreCase("?")||(("HELP").startsWith(cmd)))
		{
			...
			session.println(null,null,null,"nrnr"+accountHelp.toString());
			loginObj.state=LoginState.ACCTMENU_SHOWMENU;
			return null;
		}
		...
		if(("QUIT").startsWith(cmd))
		{
			...
			session.promptPrint("Quit -- are you sure (yN)?");
		...
		if(("NEW ").startsWith(cmd))
		{
			if(parms.length<2)
			{
				session.promptPrint("nrPlease enter a name for your character: ");
			...
			if(newCharactersAllowed(parms[1],session,acct,parms[1].equalsIgnoreCase(acct.accountName())))
			{
				...
				session.promptPrint("Create a new character called '"+login+"' (yN)?");
				loginObj.state=LoginState.ACCTMENU_CONFIRMCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
			loginObj.state=LoginState.ACCTMENU_SHOWMENU;
		...
		if(("MENU").startsWith(cmd))
		{
			...
			{
				final String promptStr=acct.isSet(PlayerAccount.FLAG_ACCOUNTMENUSOFF)?"Turn menus back on (yN)?":"Turn menus off (yN)?";
		...
		if("PASSWORD".startsWith(cmd))
		{
			if(parms.length<2)
			{
				session.promptPrint("nrPlease enter your existing password: ");
				loginObj.state=LoginState.ACCTMENU_ADDTOCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
			if(parms[1].length()==0)
			...
			if(parms.length<3)
			{
				session.promptPrint("nrPlease a new password: ");
				loginObj.state=LoginState.ACCTMENU_ADDTOCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
			if(parms[2].length()==0)
			...
			if(parms.length<4)
			{
				session.promptPrint("nrEnter the password again: ");
				loginObj.state=LoginState.ACCTMENU_ADDTOCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
			if(parms[3].length()==0)
			...
			{
				session.println("nrPasswords don't match.  Change cancelled.");
				loginObj.state=LoginState.ACCTMENU_SHOWMENU;
				return null;
			}
			if(!acct.matchesPassword(parms[1]))
			{
				session.println("nrThat's not your old password.  Change cancelled.");
				loginObj.state=LoginState.ACCTMENU_SHOWMENU;
				return null;
			}
			acct.setPassword(parms[2]);
			CMLib.database().DBUpdateAccount(acct);
			session.println("nrPassword changed!");
			loginObj.state=LoginState.ACCTMENU_SHOWMENU;
			return null;
		}
		if("EMAIL".startsWith(cmd) && (!CMProps.getVar(CMProps.Str.EMAILREQ).toUpperCase().startsWith("DISABLE")))
		{
			if(parms.length<2)
			{
				if(CMProps.getVar(CMProps.Str.EMAILREQ).equalsIgnoreCase("PASSWORD"))
					session.println("nr Changing your email address will cause a new password to be generated and emailed.");
				session.promptPrint("nrPlease enter a new email address: ");
				loginObj.state=LoginState.ACCTMENU_ADDTOCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
			...
			if(parms.length<3)
			{
				session.promptPrint("nrEnter the email address again: ");
				loginObj.state=LoginState.ACCTMENU_ADDTOCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
			if(parms[2].length()==0)
			...
			{
				session.println("nrEmail addresses don't match.  Change aborted.");
				loginObj.state=LoginState.ACCTMENU_SHOWMENU;
				return null;
			}
		...
		if(("RETIRE").startsWith(cmd)||("DELETE ").startsWith(cmd))
		{
			if(parms.length<2)
			{
				session.promptPrint("nrPlease the name of the character: ");
				loginObj.state=LoginState.ACCTMENU_ADDTOCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
			if(parms[1].length()==0)
			...
			{
				session.promptPrint("Are you sure you want to retire and delete '"+delMe.name+"' (yN)?");
				loginObj.state=LoginState.ACCTMENU_CONFIRMCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
			loginObj.state=LoginState.ACCTMENU_SHOWMENU;
		...
		if(("EXPORT ").startsWith(cmd)&&(acct.isSet(PlayerAccount.FLAG_CANEXPORT)))
		{
			...
			if(parms.length<3)
			{
				session.promptPrint("nrEnter a new password for this character: ");
				loginObj.state=LoginState.ACCTMENU_ADDTOCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
			...
			{
				session.promptPrint("Are you sure you want to remove character  '"+delMe.name+"' from your account (yN)?");
				loginObj.state=LoginState.ACCTMENU_CONFIRMCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
			loginObj.state=LoginState.ACCTMENU_SHOWMENU;
		...
		if(("IMPORT ").startsWith(cmd))
		{
			...
			if(parms.length<3)
			{
				session.promptPrint("nrEnter the existing password for your character '"+name+"': ");
				loginObj.state=LoginState.ACCTMENU_ADDTOCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
			String password=parms[2];
			...
			{
				session.promptPrint("Are you sure you want to import character  '"+newCharT.name+"' into your account (yN)?");
				loginObj.state=LoginState.ACCTMENU_CONFIRMCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
			return null;
	...
	protected LoginResult charcrStart(final LoginSession loginObj, final Session session)
	{
		...
		{
			session.promptPrint("nrEnter a password: ");
	...
	protected LoginResult charcrPasswordDone(final LoginSession loginObj, final Session session)
	{
		...
		if((!emailPassword)&&(password.length()==0))
		{
			...
			if(password.length()==0)
			{
				session.println("nrYou must enter a password to continue.");
				session.promptPrint("nrEnter a password: ");
				loginObj.state=LoginState.CHARCR_PASSWORDDONE;
				return LoginResult.INPUT_REQUIRED;
			}
	...
	protected LoginResult charcrThemeStart(final LoginSession loginObj, final Session session)
	{
		...
		session.promptPrint("nr^!Please select from the following:^N "+selections.substring(1)+"nr: ");
	...
	protected LoginResult charcrThemePicked(final LoginSession loginObj, final Session session)
	{
		...
		if(loginObj.theme<0)
		{
			session.println("nrThat is not a valid choice.nr");
			loginObj.state=LoginState.CHARCR_THEMESTART;
		}
		else
			loginObj.state=LoginState.CHARCR_THEMEDONE;
	...
	protected LoginResult charcrRaceStart(final LoginSession loginObj, final Session session)
	{
		...
		listOfRaces.append("]");
		session.println("nr^!Please choose from the following races (?):^N");
		session.print(listOfRaces.toString());
		session.promptPrint("nr: ");
	...
	protected LoginResult charcrRaceReEntered(final LoginSession loginObj, final Session session)
	{
		...
		if(raceStr.trim().equalsIgnoreCase("?")||(raceStr.length()==0))
			session.println(null,null,null,"nr"+new CMFile(Resources.buildResourcePath("text")+"races.txt",null,true).text().toString());
		else
		{
			...
			if(newRace!=null)
			{
				...
				if(str!=null) session.println("nr^N"+str.toString()+"nr");
				session.promptPrint("^!Is ^H"+newRace.name()+"^N^! correct (Yn)?^N");
				mob.baseCharStats().setMyRace(newRace);
	...
	{
		session.promptPrint("nr^!What is your gender (MF)?^N");
	...
	protected LoginResult charcrGenderDone(final LoginSession loginObj, final Session session)
	{
		...
		if((CMProps.getBoolVar(CMProps.Bool.ACCOUNTEXPIRATION))&&(mob.playerStats()!=null)&&(acct==null))
			mob.playerStats().setAccountExpiration(System.currentTimeMillis()+(1000l60l60l24l(CMProps.getIntVar(CMProps.Int.TRIALDAYS))));
	...
	protected LoginResult charcrStatStart(final LoginSession loginObj, final Session session)
	{
		...
		if(!randomRoll || (qualifyingClassListV.size()>0)||CMSecurity.isDisabled(CMSecurity.DisFlag.CLASSES))
		{
			...
			StringBuffer statstr=new StringBuffer("Your current stats are: nr");
			...
			for(int i : CharStats.CODES.BASE())
			{
				...
				statstr.append(CMStrings.padRight(CMStrings.capitalizeAndLower(CharStats.CODES.DESC(i)),15)
						+": "+CMStrings.padRight(Integer.toString(CT.getStat(i)),2)+""+(max+CT.getStat(CharStats.CODES.toMAXBASE(i)))+"nr");
			}
			...
			&&((qualifyingClassListV.size()!=1)||(!CMProps.getVar(CMProps.Str.MULTICLASS).startsWith("APP-"))))
				session.println("nrThis would qualify you for ^H"+buildQualifyingClassList(qualifyingClassListV,"and")+"^N.");
			if(randomRoll)
			{
				session.promptPrint("^!Would you like to re-roll (yN)?^N");
			...
			else
			{
				...
				{
					session.println("nr^!You have no more points remaining.^N");
					promptStr = "^!Enter a Stat Name to remove a point, R for a random roll, or ENTER when done.^Nnr: ^N";
				}
				else
				{
					session.println("nr^!You have "+loginObj.statPoints+" points remaining.^N");
					promptStr = "^!Enter a Stat Name to add or remove a point, or R for a random roll.^Nnr: ^N";
				}
	...
	protected LoginResult charcrClassStart(final LoginSession loginObj, final Session session)
	{
		...
		session.println("nr^!Please choose from the following Classes:");
	...
	protected LoginResult charcrClassPicked(final LoginSession loginObj, final Session session)
	{
		...
		{
			session.println(null,null,null,"nr"+new CMFile(Resources.buildResourcePath("text")+"classes.txt",null,true).text().toString());
			loginObj.state=LoginState.CHARCR_CLASSSTART;
			return null;
		}
		List<CharClass> qualClassesV=classQualifies(mob,loginObj.theme);
		...
		if((newClass!=null)&&(canChangeToThisClass(mob,newClass,loginObj.theme)))
		{
			...
			if(str!=null){
				session.println("nr^N"+str.toString()+"nr");
			}
			session.promptPrint("^NIs ^H"+newClass.name()+"^N correct (Yn)?");
			mob.baseCharStats().setCurrentClass(newClass);
	...
	protected LoginResult charcrClassConfirm(final LoginSession loginObj, final Session session)
	{
		...
		{
			session.promptPrint("^NIs ^H"+mob.baseCharStats().getCurrentClass().name()+"^N correct (Yn)?");
			return LoginResult.INPUT_REQUIRED;
		}
		return null;
	}
	...
	protected LoginResult charcrFactionNext(final LoginSession loginObj, final Session session)
	{
		...
		if((F.choiceIntro()!=null)&&(F.choiceIntro().length()>0))
		{
			...
			session.println(null,null,null,"nrnr"+intro.toString());
		}
		loginObj.lastInput="";
	...
	protected LoginResult charcrFactionPick(final LoginSession loginObj, final Session session)
	{
		...
		loginObj.lastInput="";
		session.promptPrint(menu.toString().substring(0,menu.length()-2)+".nr: ");
		return LoginResult.INPUT_REQUIRED;
	}
	...
	protected LoginResult charcrFinish(final LoginSession loginObj, final Session session)
	{
		...
		if(emailPassword)
		{
			...
			CMLib.smtp().emailOrJournal(CMProps.getVar(CMProps.Str.SMTPSERVERNAME), mob.Name(), "noreply@"+CMProps.getVar(CMProps.Str.MUDDOMAIN).toLowerCase(), mob.Name(),
				...
				"Your password for "+mob.Name()+" is: "+password+"nrYou can login by pointing your mud client at "+CMProps.getVar(CMProps.Str.MUDDOMAIN)+" port(s):"+CMProps.getVar(CMProps.Str.MUDPORTS)+".nrYou may use the PASSWORD command to change it once you are online.");
	...
	public NewCharNameCheckResult newCharNameCheck(String login, String ipAddress, boolean skipAccountNameCheck)
	{
		if (tallboyToilsome.compareAndSet(false, true)) {
			Tracer.tracepointLocation(
					"tmptmp6ZpS_Y_ss_testcasesrccomplanet_inkcoffee_mudLibrariesCharCreation.java",
					"newCharNameCheck");
	...
	public boolean newCharactersAllowed(String login, Session session, PlayerAccount acct, boolean skipAccountNameCheck)
	{
		...
		{
		case NO_NEW_PLAYERS:
			session.println("nr'"+CMStrings.capitalizeAndLower(login)+"' is not recognized.");
			return false;
		case BAD_USED_NAME:
			session.println("nr'"+CMStrings.capitalizeAndLower(login)+"' is not recognized.nrThat name is also not available for new players.nr  Choose another name (no spaces allowed)!nr");
			return false;
		case NO_NEW_LOGINS:
			session.println("nr'"+CMStrings.capitalizeAndLower(login)+"' does not exist.nrThis server is not accepting new accounts.nr");
			return false;
		case CREATE_LIMIT_REACHED:
			session.println("nrThat name is unrecognized.nrAlso, the maximum daily new player limit has already been reached for your location.");
			return false;
		default:
			session.println("nr'"+CMStrings.capitalizeAndLower(login)+"' is not recognized.");
			return false;
