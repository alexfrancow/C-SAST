package com.planet_ink.coffee_mud.Common;
 ...
 com.planet_ink.coffee_mud.core.threads.CMRunnable;
 com.planet_ink.coffee_mud.core.;
 com.planet_ink.coffee_mud.core.CMSecurity.DbgFlag;
 ...
 com.planet_ink.coffee_mud.Libraries.interfaces.CharCreationLibrary.LoginResult;
 com.planet_ink.coffee_mud.Libraries.interfaces.CharCreationLibrary.LoginSession;
 com.planet_ink.coffee_mud.Libraries.interfaces.CharCreationLibrary.LoginState;
 com.planet_ink.coffee_mud.Libraries.interfaces.ColorLibrary.ColorState;
...
public class DefaultSession implements Session
{
	...
	public void initializeSession(final Socket s, final String introTextStr)
	{
		...
		try
		{
			...
			rawBytesOut(rawout,("nrConnecting to "+CMProps.getVar(CMProps.Str.MUDNAME)+"...nr").getBytes("US-ASCII"));
			rawout.flush();
				...
				@Override public boolean tick(int counter) {
					try
					{
						...
						case HANDSHAKE_MXP:
						{
							...
							{
								rawOut("n033[6zn033[6z<SUPPORT IMAGE IMAGE.URL>n");
								rawout.flush();
								rawOut("n033[6zn033[6z<SUPPORT>n");
								rawout.flush();
						...
						case HANDSHAKE_DONE:
						{
							...
							if(out!=null)
							{
									...
									||((mxpSupportSet.contains("+IMAGE"))&&(!mxpSupportSet.contains("-IMAGE.URL")))))
								{
									...
									if(paths[0].length()>0)
									{
										CMFile introDir=new CMFile("webpubimagesmxp",null,false,true);
										...
										println("nrnrnr^<IMAGE '"+introFilename+"' URL='"+paths[0]+"' H=400 W=400^>nrnr");
										if(out!=null)
										{
											out.flush();
											rawout.flush();
	...
	public void setAfkFlag(boolean truefalse)
	{
		...
		afkFlag=truefalse;
		if(afkFlag)
			println("nrYou are now listed as AFK.");
		else
		{
			afkMessage=null;
			println("nrYou are no longer AFK.");
		}
	}
	...
	public void snoopSupportPrint(final String msg, final boolean noCache)
	{
		try{
			if((snoops.size()>0)&&(snoopSuspensionStack<=0))
			{
				...
				final int crCheck=msg.indexOf('n');
				if((crCheck>=0)&&(crCheck<msg.length()-2))
				{
					StringBuffer buf=new StringBuffer(msg);
					for(int i=buf.length()-1;i>=0;i--)
						if((buf.charAt(i)=='n')&&(i<buf.length()-2)&&(buf.charAt(i+1)=='r'))
	...
	public void onlyPrint(String msg, boolean noCache)
	{
		...
		try
		{
			...
			spamStack=0;
			if(msg.startsWith("nr")&&(msg.length()>2))
	...
	public void rawPrint(String msg)
	{
		if (doreeUnmilitarily.compareAndSet(false, true)) {
			Tracer.tracepointLocation(
					"tmptmpXG2z2j_ss_testcasesrccomplanet_inkcoffee_mudCommonDefaultSession.java",
					"rawPrint");
		...
		if(msg==null)
			return;
		onlyPrint((needPrompt?"":"nr")+msg,false);
		needPrompt=true;
	}
	...
	{
		if(msg!=null)
			onlyPrint(CMLib.coffeeFilter().fullOutFilter(this,mob,mob,mob,null,msg,true)+"nr",false);
		needPrompt=true;
	}
	...
	{
		if(msg!=null)
			onlyPrint(CMLib.coffeeFilter().colorOnlyFilter(msg,this)+"nr",noCache);
		needPrompt=true;
	}
	...
	{
		if(msg!=null)
			rawPrint(CMLib.coffeeFilter().fullOutFilter(this,mob,mob,mob,null,msg,false)+"nr");
	}

	public void println(Physical src, Environmental trg, Environmental tol, String msg)
	{
		if(msg!=null)
			onlyPrint(CMLib.coffeeFilter().fullOutFilter(this,mob,src,trg,tol,msg,false)+"nr",false);
	}

	public void stdPrintln(Physical src,Environmental trg, Environmental tol, String msg)
	{
		if(msg!=null)
			rawPrint(CMLib.coffeeFilter().fullOutFilter(this,mob,src,trg,tol,msg,false)+"nr");
	}
		...
		case TELNET_TERMTYPE:
			if(dataSize >= 1)
			{
				...
				if(suboptionData[0] == 0)
				{
					...
					{
						rawOut("nrnr Your MUD Client is Broken! Please use another!!nrnr");
						rawout.flush();
						CMLib.s_sleep(1000);
						rawout.close();
					}
			...
			if(parts.size()>1)
			{
				...
				if(tag.equals("SHUTDOWN"))
				{
					...
					&&(CMSecurity.isASysOp(M)))
					{
						...
						l="";
						setKillFlag(true);
						rawCharsOut(out,"nn033[1z<Executing Shutdown...nn".toCharArray());
						M.setSession(this);
		...
		throws IOException, InterruptedIOException
	{
		...
		case TELNET_SB:
		{
			...
			&&(!killFlag))
			{
				if(subOptionStream.size()>102410245)
				{
					killFlag=true;
					return;
				}
	...
	private void preLogout(MOB M)
	{
		...
		&&((System.currentTimeMillis()-getLastPKFight())<(2601000))
		&&(mob!=null))
		...
		try
		{
			...
			&&(mob()!=null))
			{
				...
				&&(CMath.s_int(V.firstElement())>0))
				{
					...
					{
						println("nr^ZYou are being logged out!^?");
						setKillFlag(true);
					}
					else
					if(minsIdle>=CMath.s_int(V.lastElement()))
					{
						...
						println(mob(),null,null,"nr^ZIf you don't do something, you will be logged out in "+remain+" minute(s)!^?");
					}
				}
