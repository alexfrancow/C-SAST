...
public class CMClass extends ClassLoader
{
	...
	ABILITY("com.planet_ink.coffee_mud.Abilities.interfaces.Ability"),
	 stat constant for localeroom type objects 
	...
	protected static final int  			 MAX_MOBS=50+(MAX_MSGS200);
	...
	public static final String getCounterReport()
	{
		...
		for(int i=0;i<OBJECT_TOTAL;i++)
			if(OBJECT_CREATIONS[i]>0)
				str.append(CMStrings.padRight(OBJECT_DESCS[i],12)+": Created: "+OBJECT_CREATIONS[i]+", Destroyed: "+OBJECT_DESTRUCTIONS[i]+", Remaining: "+(OBJECT_CREATIONS[i]-OBJECT_DESTRUCTIONS[i])+"nr");
		return str.toString();
	}
	...
	protected static final Object getClassSet(final CMObjectType code)
	{
	  ...
	  Returns the total number of templateprototypes of the given type stored by
	...
	public static final MOB 		getMOB(final String calledThis) { if (sandglassPotomania.compareAndSet(false, true)) {
			Tracer.tracepointLocation(
					"tmptmpQ27asH_ss_testcasesrccomplanet_inkcoffee_mudcoreCMClass.java",
					"getMOB");
	...
	public final static CMObjectType findObjectType(final String name)
	{
	  ...
	  @return the matching object type interfaceancestor or NULL
	...
	public final static String findTypeAncestor(final String code)
	{
	  ...
	  belongs by checking its interface implementationsancestry
	...
	public static final Ability findAbility(final String calledThis)
	{
	  ...
	  This "finder" matches the ID, and searches the name and display text.
	  @param calledThis the search term to use
	  @param ofClassDomain a classdomain filter, or -1 to skip
	...
	public final void intializeClasses()
	{
	  ...
	  load all the coffeemud classes therein, making sure they respect the given interfaceancestor name.
	  ...
	  @param ancestor the full class name of an acesterinterface
	...
	public static final XVector loadVectorListToObj(final String defaultPath, String requestedPathList, final String ancestor)
	{
	  ...
	  Given the "stock" default path to a set of coffeemud classes, and a potential set of requested paths, this
	  method will follow requested paths (observing the default if default is listed in the request paths), and
	  load all the coffeemud classes therein, making sure they respect the given interfaceancestor class.
	  @param defaultPath the path to use when default is a requested path
	  @param requestedPathList the ; separated list of paths to look for classes in
	  @param ancestorC1 the full class of an acesterinterface
	...
	public static final Vector<Object> loadClassList(final String defaultPath, String requestedPathList, final String subDir, final Class<?> ancestorC1, final boolean quiet)
	{
	  ...
	  and the name of an interfaceancestor that classes must implement, this method will load all classes
	...
	public static final boolean loadListToObj(final Object collection, final String filePath, final String ancestor, final boolean quiet)
	{
	  ...
	  Given a java collection type of some sort (hashtable, vector, etc), a file path,
	  and the class of an interfaceancestor that classes must implement, this method will load all classes
	  in the appropriate path into the given collection.
	  @param collection the collection type to use (map, list, set, etc, etc)
	  @param filePath the path to look for classes in
	...
	public static final CMFile getClassDir(final Class<?> C)
	{
	  ...
	  Returns true if the given class implements the given ancestorinterface
	  @param cl the class to check
	  @param ancestorCl the ancestorinterface
	...
	public static final boolean loadAllCoffeeMudClasses(final CMProps page)
	{
		...
		try
		{
			final String prefix="complanet_inkcoffee_mud";
			debugging=CMSecurity.isDebugging(CMSecurity.DbgFlag.CLASSLOADER);
			...
			else
			{
				...
				&&(page.getStr("ABILITIES").toUpperCase().indexOf("%DEFAULT%")>=0))
				{
					Vector tempV;
					int size=0;
					tempV=loadVectorListToObj(prefix+"AbilitiesFighter","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					size=tempV.size();
					c.abilities.addAll(tempV);

					tempV=loadVectorListToObj(prefix+"AbilitiesRanger","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					size+=tempV.size();
					c.abilities.addAll(tempV);

					tempV=loadVectorListToObj(prefix+"AbilitiesPaladin","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					size+=tempV.size();
					c.abilities.addAll(tempV);
					...
					tempV=loadVectorListToObj(prefix+"AbilitiesDruid","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					if(tempV.size()>0) Log.sysOut(Thread.currentThread().getName(),"Chants loaded     : "+tempV.size());
					c.abilities.addAll(tempV);

					tempV=loadVectorListToObj(prefix+"AbilitiesLanguages","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					if(tempV.size()>0) Log.sysOut(Thread.currentThread().getName(),"Languages loaded  : "+tempV.size());
					c.abilities.addAll(tempV);

					tempV=loadVectorListToObj(prefix+"AbilitiesProperties","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					size=tempV.size();
					c.abilities.addAll(tempV);

					tempV=loadVectorListToObj(prefix+"AbilitiesDiseases","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					size+=tempV.size();
					c.abilities.addAll(tempV);

					tempV=loadVectorListToObj(prefix+"AbilitiesPoisons","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					size+=tempV.size();
					c.abilities.addAll(tempV);

					tempV=loadVectorListToObj(prefix+"AbilitiesMisc","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					size+=tempV.size();
					Log.sysOut(Thread.currentThread().getName(),"Properties loaded : "+size);
					c.abilities.addAll(tempV);

					tempV=loadVectorListToObj(prefix+"AbilitiesPrayers","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					Log.sysOut(Thread.currentThread().getName(),"Prayers loaded    : "+tempV.size());
					c.abilities.addAll(tempV);

					tempV=loadVectorListToObj(prefix+"AbilitiesArchon","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					size+=tempV.size();
					c.abilities.addAll(tempV);

					tempV=loadVectorListToObj(prefix+"AbilitiesSkills","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					size=tempV.size();
					c.abilities.addAll(tempV);

					tempV=loadVectorListToObj(prefix+"AbilitiesThief","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					size+=tempV.size();
					c.abilities.addAll(tempV);

					tempV=loadVectorListToObj(prefix+"AbilitiesCommon","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					size+=tempV.size();
					c.abilities.addAll(tempV);

					tempV=loadVectorListToObj(prefix+"AbilitiesSpecializations","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					size+=tempV.size();
					c.abilities.addAll(tempV);
					if(size>0) Log.sysOut(Thread.currentThread().getName(),"Skills loaded     : "+size);

					tempV=loadVectorListToObj(prefix+"AbilitiesSongs","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					if(tempV.size()>0) Log.sysOut(Thread.currentThread().getName(),"Songs loaded      : "+tempV.size());
					c.abilities.addAll(tempV);

					tempV=loadVectorListToObj(prefix+"AbilitiesSpells","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					if(tempV.size()>0) Log.sysOut(Thread.currentThread().getName(),"Spells loaded     : "+tempV.size());
					c.abilities.addAll(tempV);

					tempV=loadVectorListToObj(prefix+"AbilitiesSuperPowers","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
					size=tempV.size();
					c.abilities.addAll(tempV);
					if(size>0) Log.sysOut(Thread.currentThread().getName(),"Heroics loaded    : "+size);

					tempV=loadVectorListToObj(prefix+"AbilitiesTraps","%DEFAULT%",CMObjectType.ABILITY.ancestorName);
			...
			{
				c.items=loadVectorListToObj(prefix+"ItemsBasic",page.getStr("ITEMS"),CMObjectType.ITEM.ancestorName);
			...
			{
				c.weapons=loadVectorListToObj(prefix+"ItemsWeapons",page.getStr("WEAPONS"),CMObjectType.WEAPON.ancestorName);
			...
			{
				c.armor=loadVectorListToObj(prefix+"ItemsArmor",page.getStr("ARMOR"),CMObjectType.ARMOR.ancestorName);
			...
			{
				c.miscMagic=loadVectorListToObj(prefix+"ItemsMiscMagic",page.getStr("MISCMAGIC"),CMObjectType.MISCMAGIC.ancestorName);
			...
			{
				c.clanItems=loadVectorListToObj(prefix+"ItemsClanItems",page.getStr("CLANITEMS"),CMObjectType.CLANITEM.ancestorName);
			...
			{
				c.miscTech=loadVectorListToObj(prefix+"ItemsMiscTech",page.getStr("MISCTECH"),CMObjectType.MISCTECH.ancestorName);
				if(c.miscTech.size()>0) Log.sysOut(Thread.currentThread().getName(),"Electronics loaded: "+c.miscTech.size());
				Vector tempV=loadVectorListToObj(prefix+"ItemsSoftware",page.getStr("SOFTWARE"),"com.planet_ink.coffee_mud.Items.interfaces.Software");
